...
|---|---|
| **P1 · Edge safety** | Screens must wrap content in `UnifyContainer` (applies `outerMargin`). Build-phase grep flags any root `.padding(0)` / `.padding(.horizontal, 0)` and top-level views not enclosed by `UnifyContainer`. SwiftLint custom rule bans `.ignoresSafeArea` on content containers. |
...
|---|---|---|
| 1 | **P1** no edge-flush; padding ≥ gutter | `rg -n '\.padding\(\s*(\.horizontal\s*,\s*)?0\s*\)' --glob '*.swift'` and flag screens: `rg -Ln 'struct \w+(Screen\|Page)' --glob '*.swift' \| xargs rg -L 'UnifyContainer'` (a screen with no container fails). |
