...
```bash
SKILL_DIR="$(find . -path '*whereto/skills/feel-pass/references' -type d 2>/dev/null | head -1)"
[ -z "$SKILL_DIR" ] && SKILL_DIR="$(find . -path '*/skills/feel-pass/references' -type d 2>/dev/null | head -1)"
# Prefer pack-relative path when this skill lives under product-planning/whereto/skills/feel-pass
