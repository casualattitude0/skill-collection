...
```bash
SKILL_DIR="$(find . -path '*whereto/skills/balance-review/references' -type d 2>/dev/null | head -1)"
[ -z "$SKILL_DIR" ] && SKILL_DIR="$(find . -path '*/skills/balance-review/references' -type d 2>/dev/null | head -1)"
# Prefer pack-relative path when this skill lives under product-planning/whereto/skills/balance-review
...
[ -z "$BDOC" ] && BDOC=$(ls -t "$ART"/*-balance-*.md "$ART"/*-economy-*.md 2>/dev/null | head -1)
GDD=$(ls -t docs/*GDD* docs/*game-design* docs/*design-doc* *.gdd.md 2>/dev/null | head -1)
[ -z "$GDD" ] && GDD=$(ls -t "$ART"/*-design-*.md 2>/dev/null | head -1)
PREV_REVIEW=$(ls -t "$ART"/*-balance-report-*.md 2>/dev/null | head -1)
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
if [ -d docs ]; then ART=docs/whereto-artifacts; else ART=whereto-artifacts; fi
mkdir -p "$ART"
```

Write the Balance Health Report (including baseline → final delta if applicable) to `{docs/whereto-artifacts or whereto-artifacts}/{user}-{branch}-balance-report-{datetime}.md`. This artifact is discoverable by downstream skills and future balance reviews.
