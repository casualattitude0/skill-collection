...
|---|---|
| **P1 · Edge safety** | **Container edge:** `custom_lint` `unify_edge_safety` flags any `Padding`/`EdgeInsets` whose horizontal inset on a scroll/scaffold body is `< spec.outerMargin`; screens must wrap content in `ResponsiveContainer`. Grep gate: `rg 'Scaffold\(' -A20 \| rg 'Padding' -c`. **Internal divider:** a `Divider()`/`VerticalDivider()` or a `Border`/`border:` side must have ≥ `DesignTokens.space3` (12) of `SizedBox`/`Padding` between the rule and the content on **each** side — the rule flags a `Divider()` whose sibling in the `Column`/`Row` is not separated by `SizedBox(height/width: >= space3)` or wrapped in `Padding(>= space3)` (a label must never sit on its divider). Grep: `rg -n 'Divider\(' -A2 lib/ \| rg -L 'SizedBox\|Padding'`. **Rounded corner:** a `ClipRRect`/`borderRadius:`/`BorderRadius.circular(r)` container must inset its child ≥ `r` (a `Padding` ≥ that radius) so the curve never clips it — grep `rg -n 'ClipRRect\|borderRadius:' -A3 lib/ \| rg -L 'Padding'`. Grep is approximate: whether a label truly sits *on* the rule, or a child is clipped by a specific corner, is a **golden/widget-test visual review** asserting no child paints within `space3` of a `Divider` or within `r` of a rounded corner. |
...
| # | Row | Command |
|---|---|---|
| 1 | **P1** edge safety (edge · divider · corner) | **Edge:** `rg -n 'Scaffold\(' -A25 lib/ \| rg -L 'ResponsiveContainer'` — screens whose body never wraps in `ResponsiveContainer` (no guaranteed outer margin). **Divider:** `rg -n 'Divider\(' -A2 lib/ \| rg -L 'SizedBox\|Padding'` — a rule with no `SizedBox`/`Padding` ≥ `space3` separating it from neighbors (a label sitting on its divider). **Corner:** `rg -n 'ClipRRect\|borderRadius:' -A3 lib/ \| rg -L 'Padding'` — rounded/clipped containers whose children lack a corner-safe inset ≥ `r`. Greps only surface candidates; whether content is actually bisected by the rule or clipped by the curve needs a **golden/widget-test visual check**. |
