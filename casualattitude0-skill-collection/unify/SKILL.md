---
name: unify
description: Enforce one design direction across UI and code. Defines a source-of-truth standard (principles, tokens, grid, component contract, audit checklist), generates UI that follows it by construction, and audits existing UI for violations — starting with "nothing touches the edge". Materializes to Web, Flutter, and SwiftUI from one core. Use when screens look casually laid out or drift apart. Not for one-off visual polish on a single component.
user_invocable: true
license: MIT
metadata:
  author: unify
