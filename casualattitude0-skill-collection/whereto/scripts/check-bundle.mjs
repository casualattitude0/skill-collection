...
for (const s of manifest.skills) {
  ...
  if (s.mode === "detached") {
    // No source to diff against — the bundle IS the only copy. Presence only.
    if (!fs.existsSync(path.join(bundleDir, "SKILL.md")))
      errors.push(`"${s.name}" (detached): skills/${s.name}/SKILL.md missing`);
    if (s.source)
      errors.push(`"${s.name}" (detached): must have source null, got "${s.source}"`);
    continue;
  }
