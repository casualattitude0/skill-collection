---
name: git-hot-fix
description: Ship an urgent fix straight to production off-cycle, then land it on every long-lived branch so it cannot reappear in the next release. Auto-detects Git Flow vs GitHub Flow, delivers via gh PRs, optionally bumps and tags. Use for "critical bug in production", "hotfix the release", "patch prod now", "the live site is broken", or backporting a fix across release branches. Not for a non-urgent bug that can wait — that is an ordinary branch and PR via github-pr-workflow.
license: MIT
metadata:
