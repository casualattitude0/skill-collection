...
description: Gate a merge behind exactly one review, then halt — the reviewing sub-agent runs a single time; Pass merges, Failed posts its rejection and stops there, nothing repaired and nothing re-checked. Use for a one-shot or no-fix review gate, where a failed verdict should end the process instead of starting repairs. Not for repeated rounds until clean (github-pr-workflow), nor for merging unreviewed (github-pr-merge).
license: MIT
metadata:
