...
description: Iterate a pull request until it is mergeable — a sub-agent reviews, failures get repaired in the code and re-reviewed for as many rounds as it takes, rejections post as event=COMMENT, and the merge lands once the verdict turns Pass. Use when review failures should be fixed automatically, or the loop should keep going until the branch is clean. Not for halting at the first failure (github-pr-workflow-only), nor for merging unreviewed (github-pr-merge).
license: MIT
metadata:
