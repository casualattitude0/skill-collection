...
description: Fix SSH authenticating to GitHub as the WRONG account when several accounts and keys share one machine. Use when git push/clone or `ssh -T` logs in as the wrong user, a per-account host alias resolves to someone else, or `Permission denied` hits a repo you know you own — and whenever IdentityFile, IdentitiesOnly, IdentityAgent, ssh-agent key order, or a `Host *` catch-all is involved. Also audits `~/.ssh/config` for hijack risk. Not for first-time SSH setup.
license: MIT
metadata:
