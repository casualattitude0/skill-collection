---
name: commitify
description: Intelligently stage and commit git changes in logical, well-named batches following Conventional Commits standards. Use whenever the user types "/commitify all" (commit everything in the repo) or "/commitify current" (commit only files related to the current chat conversation). Always invoke this skill on those exact commands. Not for writing one commit message by hand, or for opening and merging PRs.
license: MIT
metadata:
...
| Command | Behavior |
|---|---|
| `/commitify all` | Stage and commit all changed files, grouped by logical change |
| `/commitify current` | Stage and commit only files related to the current chat |
...
If there are remaining files in scope (all remaining files for `/commitify all`,
or remaining relevant files for `/commitify current`), continue grouping and
committing. Stop when:
- `/commitify all`: working tree is clean (`nothing to commit`)
- `/commitify current`: all conversation-relevant files are committed
...
If `/commitify current` left some files uncommitted (not relevant to the
